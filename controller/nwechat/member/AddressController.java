...
import com.cb.vo.order.OrderProcessVo;
...
@Controller("nwechat-addressController")
@RequestMapping("/nwechat/member/address")
public class AddressController extends CommonController {
        ...
        if (member != null) {
            ...
            if (StringUtil.isNotBlank(province)) {
                ...
                if (addressDatabase != null) {
                    ...
                    addressVo.setWarehouse(warehouse.getName());
                }
            }
    ...
    public String list() {
        ...
        Member member = memberService.findById(ShiroSecurityUtil.getCurrentMemberId());
        if (member != null) {
            List addressVolist = ListUtil.copyPropertiesInList(AddressVo.class, member.getAddresses());
            result.put("addresses", addressVolist);
            result.success();
        }
        return result.toJsonString();
    }
